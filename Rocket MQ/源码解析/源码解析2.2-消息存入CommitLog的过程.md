...
public MappedFile getLastMappedFile(final long startOffset, boolean needCreate) {
  ...
  if (this.allocateMappedFileService != null) {
    //真正的创建CommitLog
    mappedFile = this.allocateMappedFileService.putRequestAndReturnMappedFile(nextFilePath, nextNextFilePath, this.mappedFileSize); //5.1和5.2
  }
...
public AppendMessageResult doAppend(final long fileFromOffset,
   ...
   *  4 + BodyLength //消息体长度 + BobyLength(重要)
   *  1 + TopicLength //Topic长度 + TopicLength(重要) 
   *  2 + PropertiesLength //Properties长度 + PropertiesLength(重要)
   */
